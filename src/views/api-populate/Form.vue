...
export default {
  setup() {
    ...
    const { data, error, isLoading, execute } = useEffect(
    ...
    function removeOtherSoftware(i) {
      itRequestRef.value = itRequestRef.value.filter(
        (val, index) => i !== index
      );
    }
